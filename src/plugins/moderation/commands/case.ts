...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            logger.error({ msg: '[ERROR] Case command error:', err: error });
...
async function handleEditCase(interaction: ChatInputCommandInteraction) {
    ...
    // Log the action
    logger.info({ msg: `[MODERATION] Case #${caseId} edited by ${interaction.user.tag}` });
}
...
async function handleDeleteCase(interaction: ChatInputCommandInteraction) {
    ...
    // Log the action
    logger.info({ msg: `[MODERATION] Case #${caseId} deleted by ${interaction.user.tag}. Reason: ${reason}` });
}
 ...
 */
export async function createModCase(guildId: string, caseInfo: {
    type: string;
    targetId: string;
    targetTag: string;
    moderatorId: string;
