        ...
        try {
            ...
            if (!user) {
                const errorEmbed = {
                    ...
                    description: 'Please specify a valid user to deafen.',
            ...
            if (member.voice.serverDeaf) {
                const errorEmbed = {
                    color: 0xFFFF00,
                    title: '[INFO] Already Deafened',
                    description: `${user.tag} is already server deafened.`,
                    timestamp: new Date().toISOString()
                };
                return interaction.reply({ embeds: [errorEmbed], flags: MessageFlags.Ephemeral });
            }
            ...
            if (user.id === interaction.user.id) {
                const errorEmbed = {
                    ...
                    description: 'You cannot deafen yourself using this command.',
                    timestamp: new Date().toISOString()
                };
                return interaction.reply({ embeds: [errorEmbed], flags: MessageFlags.Ephemeral });
            }
            ...
            if (user.id === interaction.client.user.id) {
                const errorEmbed = {
                    ...
                    description: 'You cannot deafen the bot.',
                    timestamp: new Date().toISOString()
                };
                return interaction.reply({ embeds: [errorEmbed], flags: MessageFlags.Ephemeral });
            }
            ...
            const successEmbed = {
                color: 0x00FF00,
                title: '[SUCCESS] User Deafened',
                description: `${user.tag} has been server deafened in **${channelName}**.`,
                fields: [
            ...
            logger.info(`[MODERATION] User ${user.tag} was voice deafened by ${interaction.user.tag}. Reason: ${reason}`);
            
        } catch (error) {
            const errorEmbed = {
                color: 0xFF0000,
                title: '[ERROR] Command Failed',
                description: 'An error occurred while trying to deafen the user.',
                fields: [
