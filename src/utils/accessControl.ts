...
import type { Interaction, GuildMember, ChatInputCommandInteraction, PermissionResolvable } from 'discord.js';

let _ownerIds: string[] | null = null;
 ...
 */
export function clearOwnerIdsCache(): void {
    _ownerIds = null;
}
 ...
 */
export function isOwnerInteraction(interaction: Interaction): boolean {
 ...
 */
export function createAccessDeniedEmbed(message = 'Only bot owners can use this command.'): EmbedBuilder {
 ...
 * @param {PermissionResolvable} permission - Permission flag (e.g., PermissionsBitField.Flags.BanMembers)
 ...
 * @param {GuildMember} member - Discord guild member
 * @param {PermissionResolvable[]} permissions - Array of permission flags
 * @returns {boolean} True if member has any of the permissions
 */
export function hasAnyPermission(member: GuildMember, permissions: PermissionResolvable[]): boolean {
 ...
 */
export function hasAllPermissions(member: GuildMember, permissions: PermissionResolvable[]): boolean {
 ...
 */
export function createPermissionDeniedEmbed(permissionName = 'the required permission'): EmbedBuilder {
    return new EmbedBuilder()
        .setColor(0xFF0000)
