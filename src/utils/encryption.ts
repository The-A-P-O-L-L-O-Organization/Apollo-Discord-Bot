...
export async function decrypt(encryptedData: string): Promise<string | object | unknown[]> {
    ...
    if (parts.length === 4) {
        ...
        [, saltB64, ivB64, authTagB64, ciphertextB64] = parts as [string, string, string, string, string];
    } else {
        throw new Error('Invalid encrypted data format');
    }
    ...
    for (const field of fields) {
        if (result[field] !== undefined && result[field] !== null) {
            try {
                ...
                logger.warn(`[ENCRYPTION] Failed to decrypt field ${field}: ${(err as Error).message}`);
            }
        }
    }
    return result;
