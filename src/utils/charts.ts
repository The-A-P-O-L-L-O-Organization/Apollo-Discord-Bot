 ...
 */
export function createBarChart(data: DataPoint[], maxWidth = 20): string {
 ...
 */
export function createPercentageBar(value: number, max: number, width = 10): string {
 ...
 */
export function createLineChart(data: DataPoint[], height = 5): string {
    if (!data || data.length === 0) {
        return 'No data available';
    }
    ...
    const labelLine = data.map(d => d.label.charAt(0)).join(' ');
 ...
 */
export function createTrendIndicator(current: number, previous: number): string {
    if (previous === 0) {
        return '➡️ New';
    }
 ...
 */
export function createTable(data: Record<string, unknown>[], columns: string[]): string {
    if (!data || data.length === 0) {
        return 'No data available';
    }
