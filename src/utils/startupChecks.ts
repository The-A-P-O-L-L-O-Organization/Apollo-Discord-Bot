 ...
 * @returns void
 */
export function warnUnverifiedPlugins(): void {
 ...
 * @param poolConfig - Pool configuration with min/max
 * @param connectionString - Postgres connection string
 * @returns Promise<void>
 */
export async function validatePostgresPoolMax(
    poolConfig: { max: number } | null | undefined,
    ...
    try {
        ...
        logger.warn(`[WARN] Could not validate Postgres pool max against max_connections: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
        await testPool.end();
    }
}

export function assertDiscordToken(token: string | undefined): void {
...
export function assertEncryptionKey(key: string | undefined): void {
    if (!key) {
        throw new Error(
...
export function assertOperatorAgreement(operator: OperatorConfig | null | undefined): void {
...
export function validateSocketToken(): void {
    const isProduction = process.env['NODE_ENV'] === 'production';
...
export async function validateRedisAuth(): Promise<void> {
    const isProduction = process.env['NODE_ENV'] === 'production';
    ...
    try {
        ...
        const err = new Error(`[FATAL] Redis authentication failed: ${error instanceof Error ? error.message : String(error)}`);
        err.cause = error;
        throw err;
    } finally {
        await redis.quit();
