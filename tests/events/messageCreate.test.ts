...
vi.mock('../../src/utils/db.js', () => ({
    ...
    getUserData: vi.fn().mockImplementation((key: string, _guildId: unknown, _userId: unknown) => {
        if (key === 'warnings') {
            return [];
        }
...
import * as mockedAutomod from '../../src/utils/automod.js';
...
const {
    ...
    checkBurstSpam,
    checkAccountAge
} = mockedAutomod as unknown as {
    getAutomodConfig: ReturnType<typeof vi.fn>;
    isExempt: ReturnType<typeof vi.fn>;
    isChannelExempt: ReturnType<typeof vi.fn>;
    checkBannedWords: ReturnType<typeof vi.fn>;
    checkInvites: ReturnType<typeof vi.fn>;
    checkLinks: ReturnType<typeof vi.fn>;
    checkMentionSpam: ReturnType<typeof vi.fn>;
    checkCapsSpam: ReturnType<typeof vi.fn>;
    checkSpam: ReturnType<typeof vi.fn>;
    checkBurstSpam: ReturnType<typeof vi.fn>;
    checkAccountAge: ReturnType<typeof vi.fn>;
};
const { appendToUserArray, getUserData } = mockedDb as unknown as {
    appendToUserArray: ReturnType<typeof vi.fn>;
    getUserData: ReturnType<typeof vi.fn>;
};
const { sendModLog } = mockedModLog as unknown as {
    sendModLog: ReturnType<typeof vi.fn>;
};
const { checkMessageAttachments } = mockedNsfw as unknown as {
    checkMessageAttachments: ReturnType<typeof vi.fn>;
};
const { checkMessageModeration } = mockedOpenai as unknown as {
    checkMessageModeration: ReturnType<typeof vi.fn>;
};

interface AutomodTestConfig {
    enabled: boolean;
    bannedWords: string[];
    maxMentions: number;
...
describe('MessageCreate Event', () => {
    ...
    describe('Message Filtering', () => {
        ...
        it('should skip if member is null', async() => {
            (mockMessage as unknown as { member: GuildMember | null }).member = null;
    ...
    describe('NSFW Filter Check', () => {
        it('should check attachments when nsfwFilter is enabled', async() => {
            ...
            getAutomodConfig.mockReturnValue(automodConfig);
            mockMessage.attachments = new Map([['att1', { id: 'att1', contentType: 'image/png' }]]) as unknown as Message['attachments'];
        ...
        it('should not check when no attachments present', async() => {
            automodConfig.nsfwFilter = true;
            getAutomodConfig.mockReturnValue(automodConfig);
            mockMessage.attachments = new Map() as unknown as Message['attachments'];
