...
import type { Message, Guild, User, TextChannel } from 'discord.js';
...
const { logEvent, createMessageEditEmbed } = mockedLogger as unknown as {
    logEvent: ReturnType<typeof vi.fn>;
    createMessageEditEmbed: ReturnType<typeof vi.fn>;
};
...
describe('MessageUpdate Event', () => {
    ...
    describe('Message Filtering', () => {
        it('should ignore DM messages', async() => {
            (newMessage as unknown as { guild: Guild | null }).guild = null;
        ...
        it('should ignore when author is null', async() => {
            (newMessage as unknown as { author: User | null }).author = null;
    ...
    describe('Partial Message Handling', () => {
        it('should fetch partial old message', async() => {
            (oldMessage as unknown as { partial: boolean }).partial = true;
            oldMessage.fetch = vi.fn().mockResolvedValue({
                ...oldMessage,
                content: 'Fetched old content',
                partial: false
            }) as unknown as Message['fetch'];
        ...
        it('should fetch partial new message', async() => {
            (newMessage as unknown as { partial: boolean }).partial = true;
        ...
        it('should use placeholder when old message fetch fails', async() => {
            (oldMessage as unknown as { partial: boolean }).partial = true;
            oldMessage.fetch = vi.fn().mockRejectedValue(new Error('Cannot fetch')) as unknown as Message['fetch'];
        ...
        it('should return early when new message fetch fails', async() => {
            (newMessage as unknown as { partial: boolean }).partial = true;
            newMessage.fetch = vi.fn().mockRejectedValue(new Error('Cannot fetch')) as unknown as Message['fetch'];
