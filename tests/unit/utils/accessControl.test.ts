...
describe('accessControl', () => {
    ...
    describe('isOwnerInteraction', () => {
        it('checks interaction user ID', () => {
            ...
            expect(accessControl.isOwnerInteraction(interaction as unknown as Interaction)).toBe(true);
        });

        it('returns false for non-owner interaction', () => {
            process.env['OWNER_IDS'] = '123456789';
            const interaction = { user: { id: '999999999' } };
            expect(accessControl.isOwnerInteraction(interaction as unknown as Interaction)).toBe(false);
        });
    });
    ...
    describe('requireOwner', () => {
        it('returns null for owner', async () => {
            process.env['OWNER_IDS'] = '123456789';
            const interaction = { user: { id: '123456789' } };
            const result = await accessControl.requireOwner(interaction as unknown as Interaction);
        ...
        it('respects ephemeral option', async () => {
            process.env['OWNER_IDS'] = '123456789';
            const interaction = { user: { id: '999999999' } };
            const result = await accessControl.requireOwner(interaction as unknown as Interaction, { ephemeral: false });
        ...
        it('uses custom message', async () => {
            process.env['OWNER_IDS'] = '123456789';
            const interaction = { user: { id: '999999999' } };
            const result = await accessControl.requireOwner(interaction as unknown as Interaction, { customMessage: 'Custom deny' });
    ...
    describe('withOwnerCheck', () => {
        it('calls executeFn for owner', async () => {
            ...
            const result = await wrapped(interaction as unknown as ChatInputCommandInteraction);
        ...
        it('returns denial for non-owner without calling executeFn', async () => {
            ...
            await wrapped(interaction as unknown as ChatInputCommandInteraction);
    ...
    describe('hasPermission', () => {
        it('returns true when member has permission', () => {
            ...
            expect(accessControl.hasPermission(member as unknown as GuildMember, 'BanMembers')).toBe(true);
        });

        it('returns false when member lacks permission', () => {
            const member = { permissions: { has: vi.fn().mockReturnValue(false) } };
            expect(accessControl.hasPermission(member as unknown as GuildMember, 'BanMembers')).toBe(false);
        });
    });

    describe('hasAnyPermission', () => {
        it('returns true when member has at least one permission', () => {
            const member = { permissions: { has: vi.fn().mockImplementation((p: string) => p === 'KickMembers') } };
            expect(accessControl.hasAnyPermission(member as unknown as GuildMember, ['BanMembers', 'KickMembers'])).toBe(true);
        });

        it('returns false when member has none of the permissions', () => {
            const member = { permissions: { has: vi.fn().mockReturnValue(false) } };
            expect(accessControl.hasAnyPermission(member as unknown as GuildMember, ['BanMembers', 'KickMembers'])).toBe(false);
        });
    });
    ...
    describe('hasAllPermissions', () => {
        it('returns true when member has all permissions', () => {
            const member = { permissions: { has: vi.fn().mockReturnValue(true) } };
            expect(accessControl.hasAllPermissions(member as unknown as GuildMember, ['BanMembers', 'KickMembers'])).toBe(true);
        });

        it('returns false when member lacks any permission', () => {
            const member = { permissions: { has: vi.fn().mockImplementation((p: string) => p === 'BanMembers') } };
            expect(accessControl.hasAllPermissions(member as unknown as GuildMember, ['BanMembers', 'KickMembers'])).toBe(false);
        });
    });
    ...
    describe('requirePermission', () => {
        it('returns null when member has permission in guild', async () => {
            ...
            const result = await accessControl.requirePermission(interaction as unknown as Interaction, 'BanMembers');
            expect(result).toBeNull();
        });
