...
describe('Remind Command', () => {
    ...
    describe('execute - Success Cases', () => {
        it('should create reminder successfully', async() => {
            ...
            vi.mocked(parseTimeString).mockReturnValue(60 * 60 * 1000); // 1 hour

            await remindCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction);
            
            expect(addReminder).toHaveBeenCalled();
            const reminderArg = vi.mocked(addReminder).mock.calls[0]![0];
        ...
        it('should reply with confirmation', async() => {
            ...
            vi.mocked(parseTimeString).mockReturnValue(60 * 60 * 1000);
    ...
    describe('execute - Error Cases', () => {
        it('should reject invalid time format', async() => {
            ...
            vi.mocked(parseTimeString).mockReturnValue(null);
        ...
        it('should reject zero duration', async() => {
            ...
            vi.mocked(parseTimeString).mockReturnValue(0);
        ...
        it('should reject negative duration', async() => {
            ...
            vi.mocked(parseTimeString).mockReturnValue(-3600000);
        ...
        it('should reject duration exceeding max', async() => {
            ...
            vi.mocked(parseTimeString).mockReturnValue(60 * 24 * 60 * 60 * 1000); // 60 days
    ...
    describe('execute - Time Parsing', () => {
        it('should pass time string to parser', async() => {
            ...
            vi.mocked(parseTimeString).mockReturnValue(9000000);
    ...
    describe('execute - Guild Context', () => {
        it('should use guild ID when in server', async() => {
            ...
            const reminderArg = vi.mocked(addReminder).mock.calls[0]![0];
            expect((reminderArg as unknown as { guildId: unknown }).guildId).toBe('987654321');
        });
        ...
        it('should use "dm" when not in server', async() => {
            ...
            const reminderArg = vi.mocked(addReminder).mock.calls[0]![0];
            expect((reminderArg as unknown as { guildId: unknown }).guildId).toBe('dm');
        });
    });
});
