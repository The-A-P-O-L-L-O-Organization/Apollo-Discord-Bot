...
describe('Logging Command', () => {
    ...
    describe('execute - status subcommand', () => {
        ...
        it('should show log channel status', async() => {
            ...
            const channelField = embed.fields.find((f: { name: string; value: string }) => f.name === 'Log Channel');
        ...
        it('should show event statuses', async() => {
            ...
            const deleteField = embed.fields.find((f: { name: string; value: string }) => f.name === 'Message Delete');
            expect(deleteField.value).toContain('Enabled');
            
            const editField = embed.fields.find((f: { name: string; value: string }) => f.name === 'Message Edit');
