...
import type { MockClient, MockCommandInteraction } from '../mocks/discord.js';

describe('Help Command', () => {
    let mockInteraction: MockCommandInteraction;
    let mockClient: MockClient;
    let mockCommands: Map<string, unknown>;
    ...
    describe('execute - Success Cases', () => {
        it('should display help embed', async() => {
            await helpCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction);
        ...
        it('should group commands by category', async() => {
            ...
            const fieldNames = embed.fields.map((f: { name: string; value: string }) => f.name);
            expect(fieldNames.some((n: string) => n.includes('Utility'))).toBe(true);
            expect(fieldNames.some((n: string) => n.includes('Moderation'))).toBe(true);
        });
        ...
        it('should show command usage format', async() => {
            ...
            const moderationField = embed.fields.find((f: { name: string; value: string }) => f.name.includes('Moderation'));
        ...
        it('should include legend for parameters', async() => {
            ...
            const legendField = embed.fields.find((f: { name: string; value: string }) => f.name === 'Legend');
        ...
        it('should include usage guide', async() => {
            ...
            const usageField = embed.fields.find((f: { name: string; value: string }) => f.name === 'How to Use Commands');
    ...
    describe('execute - Edge Cases', () => {
        ...
        it('should handle commands without category', async() => {
            ...
            const uncategorizedField = embed.fields.find((f: { name: string; value: string }) => f.name.includes('Uncategorized'));
        ...
        it('should handle commands without options', async() => {
            ...
            const utilityField = embed.fields.find((f: { name: string; value: string }) => f.name.includes('Utility'));
    ...
    describe('execute - Sorting', () => {
        it('should sort Utility category first', async() => {
            ...
            const categoryFields = embed.fields.filter((f: { name: string; value: string }) => f.name.includes('[CATEGORY]'));
