...
describe('Warn Command', () => {
    ...
    beforeEach(() => {
        ...
        const mockData2 = {};
                vi.mocked(getGuildData).mockResolvedValue(mockData2 as unknown as Awaited<ReturnType<typeof getGuildData>>);
    });
    ...
    describe('Command Metadata', () => {
        ...
        it('should have required user and reason options', () => {
            const userOption = warnCommand.options.find((o: { name: string }) => o.name === 'user');
            const reasonOption = warnCommand.options.find((o: { name: string }) => o.name === 'reason');
    ...
    describe('execute - Success Cases', () => {
        it('should warn user successfully', async() => {
            await warnCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction);
        ...
        it('should include warning count in response', async() => {
            ...
            const countField = embed.fields.find((f: { name: string; value: string }) => f.name === 'Total Warnings');
        ...
        it('should handle DM failure gracefully', async() => {
            ...
            const dmField = replyCall.embeds[0].fields.find((f: { name: string; value: string }) => f.name === 'DM Sent');
    ...
    describe('execute - Auto Punishment', () => {
        it('should auto-mute at 3 warnings', async() => {
            ...
            const autoPunishField = replyCall.embeds[0].fields.find(
                (f: { name: string; value: string }) => f.name.includes('Auto-Punishment')
            );
        ...
        it('should handle auto-mute failure gracefully', async() => {
            const mockData8 = [
                ...
                { id: '2', active: true }
                        ...
                        vi.mocked(getUserData).mockResolvedValue(mockData8 as unknown as Awaited<ReturnType<typeof getUserData>>);
        ...
        it('should show next threshold in response', async() => {
            const mockData9: Array<unknown> = [];
                        vi.mocked(getUserData).mockResolvedValue(mockData9 as unknown as Awaited<ReturnType<typeof getUserData>>);
            
            await warnCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction);
            
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const thresholdField = replyCall.embeds[0].fields.find(
                (f: { name: string; value: string }) => f.name === 'Next Threshold'
            );
            ...
            const mockData10 = {
                thresholds: {
                    ...
                    ban: 15
                        ...
                        vi.mocked(getGuildData).mockResolvedValue(mockData10 as unknown as Awaited<ReturnType<typeof getGuildData>>);
            
            const mockData11 = [
                { id: '1', active: true },
                { id: '2', active: true }
                        ];
                        vi.mocked(getUserData).mockResolvedValue(mockData11 as unknown as Awaited<ReturnType<typeof getUserData>>);
    ...
    describe('execute - Warning ID Generation', () => {
        ...
        it('should include warning ID in response', async() => {
            ...
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const idField = replyCall.embeds[0].fields.find((f: { name: string; value: string }) => f.name === 'Warning ID');
