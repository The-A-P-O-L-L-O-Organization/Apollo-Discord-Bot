...
describe('Ticket Setup Command', () => {
    ...
    let mockCategory: MockTextChannel;
    let mockRole: { id: string; name: string; toString: () => string };
    ...
    describe('execute - Status Subcommand', () => {
        ...
        it('should display configured category', async() => {
            ...
            const categoryField = fields.find((f: { name: string; value: string }) => f.name === 'Ticket Category');
        ...
        it('should display configured support role', async() => {
            ...
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const fields = replyCall.embeds[0].data.fields;
            const roleField = fields.find((f: { name: string; value: string }) => f.name === 'Support Role');
        ...
        it('should display panel link when configured', async() => {
            ...
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const fields = replyCall.embeds[0].data.fields;
            const panelField = fields.find((f: { name: string; value: string }) => f.name === 'Ticket Panel');
