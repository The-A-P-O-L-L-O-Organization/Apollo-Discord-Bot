...
describe('Ping Command', () => {
    ...
    describe('execute', () => {
        ...
        it('should include latency information in embed', async() => {
            ...
            const latencyField = embedData.fields.find((f: { name: string; value: string }) => f.name === 'Round-Trip Latency');
            expect(latencyField).toBeTruthy();
            expect(latencyField.value).toContain('ms');
        });

        it('should include API latency', async() => {
            (mockInteraction.client.ws as { ping: number }).ping = 75;
            await pingCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction);
            
            const replyCall = mockInteraction.editReply.mock.calls[0]![0];
            const embedData = replyCall.embeds[0].toJSON();
            
            const apiField = embedData.fields.find((f: { name: string; value: string }) => f.name === 'API Latency');
        ...
        it('should show EXCELLENT status for low latency', async() => {
            ...
            const statusField = embedData.fields.find((f: { name: string; value: string }) => f.name === 'Status');
