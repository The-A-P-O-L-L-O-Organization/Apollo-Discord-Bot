...
describe('Userinfo Command', () => {
    ...
    describe('Command Metadata', () => {
        ...
        it('should have correct options', () => {
            ...
            const userOption = userinfoCommand.options.find((o: { name: string }) => o.name === 'user');
    ...
    describe('execute - Success Cases', () => {
        ...
        it('should display username field', async() => {
            ...
            const usernameField = fields.find((f: { name: string; value: string }) => f.name === 'Username');
        ...
        it('should display user ID field', async() => {
            ...
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const fields = replyCall.embeds[0].data.fields;
            const idField = fields.find((f: { name: string; value: string }) => f.name === 'User ID');
        ...
        it('should display bot status field', async() => {
            ...
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const fields = replyCall.embeds[0].data.fields;
            const botField = fields.find((f: { name: string; value: string }) => f.name === 'Bot');
        ...
        it('should display account creation date', async() => {
            ...
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const fields = replyCall.embeds[0].data.fields;
            const createdField = fields.find((f: { name: string; value: string }) => f.name === 'Account Created');
        ...
        it('should display server join date', async() => {
            ...
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const fields = replyCall.embeds[0].data.fields;
            const joinedField = fields.find((f: { name: string; value: string }) => f.name === 'Joined Server');
        ...
        it('should display top role', async() => {
            ...
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const fields = replyCall.embeds[0].data.fields;
            const roleField = fields.find((f: { name: string; value: string }) => f.name === 'Top Role');
        ...
        it('should display role count', async() => {
            ...
            const replyCall = mockInteraction.reply.mock.calls[0]![0];
            const fields = replyCall.embeds[0].data.fields;
            const roleCountField = fields.find((f: { name: string; value: string }) => f.name === 'Role Count');
    ...
    describe('execute - Status Indicators', () => {
        it('should show online status', async() => {
            (targetMember as unknown as { presence: unknown }).presence = { status: 'online' } as unknown as MockGuildMember['presence'];
        ...
        it('should show idle status', async() => {
            (targetMember as unknown as { presence: unknown }).presence = { status: 'idle' } as unknown as MockGuildMember['presence'];
        ...
        it('should show dnd status', async() => {
            (targetMember as unknown as { presence: unknown }).presence = { status: 'dnd' } as unknown as MockGuildMember['presence'];
        ...
        it('should show offline status', async() => {
            (targetMember as unknown as { presence: unknown }).presence = { status: 'offline' } as unknown as MockGuildMember['presence'];
        ...
        it('should show offline when presence is null', async() => {
            (targetMember as unknown as { presence: unknown }).presence = null;
