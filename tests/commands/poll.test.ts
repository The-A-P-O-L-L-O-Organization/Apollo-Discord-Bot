...
import type { MockCommandInteraction, MockMessage, MockTextChannel } from '../mocks/discord.js';
...
describe('Poll Command', () => {
    ...
    describe('execute - Success Cases', () => {
        it('should create poll with two options', async() => {
            ...
            await pollCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction);
        ...
        it('should handle poll with duration', async() => {
            ...
            const appendCall = vi.mocked(appendToGuildArray).mock.calls[0]!;
            expect(appendCall[0]).toBe('polls');
            expect((appendCall[3] as unknown as { endTime: number }).endTime).toBeDefined();
        });
    ...
    describe('execute - Duration Parsing', () => {
        it('should parse minutes correctly', async() => {
            ...
            const appendCall = vi.mocked(appendToGuildArray).mock.calls[0]!;
            const pollData = (appendCall[3] as unknown as { endTime: number });
