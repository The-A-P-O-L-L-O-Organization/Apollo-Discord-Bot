...
interface ReportGuild { id: string; channels: { cache: { get: ReportMockFn } }; }
interface ReportUser { id: string; tag: string; }
interface ReportCtxInteraction { user: ReportUser; guild: ReportGuild; channel: ReportChannel; options: { getMessage: ReportMockFn }; showModal: ReportMockFn; reply: ReportMockFn; message: ReportMessage; }
interface ReportModalInteraction { isModalSubmit: ReportMockFn; customId: string; user: ReportUser; guild: ReportGuild; channel: ReportChannel; message: { reference: { messageId: string } | undefined }; fields: { getTextInputValue: ReportMockFn }; reply: ReportMockFn; }
interface ReportLogChannel { id: string; send: ReportMockFn; }
...
describe('Report Command', () => {
    let interaction: ReportCtxInteraction;
    let message: ReportMessage;
    let channel: ReportChannel;
    let guild: ReportGuild;
    let user: ReportUser;
    ...
    describe('Message Context Menu Execution', () => {
        it('should show modal when message is found', async() => {
            ...
            await reportCommand.execute(interaction as unknown as MessageContextMenuCommandInteraction);
...
describe('Report Handler', () => {
    let interaction: ReportModalInteraction;
    let client: unknown;
    let message: ReportMessage;
    let channel: ReportChannel;
    let guild: ReportGuild;
    let user: ReportUser;
    let author: ReportAuthor;
    let logChannel: ReportLogChannel;
    ...
    beforeEach(() => {
        ...
        // Mock generateId
        vi.mocked(generateId).mockReturnValue('report123');
    ...
    describe('Modal Submission Handling', () => {
        it('should return false for non-modal interactions', async() => {
            ...
            const result = await handleReportSubmission(interaction as unknown as Parameters<typeof handleReportSubmission>[0], client);
        ...
        it('should handle missing message reference', async() => {
            interaction.message.reference = undefined;
        ...
        it('should handle messages without text content', async() => {
            ...
            const messageField = embed.fields.find((f: { name: string; value: string }) => f.name === '[INFO] Reported Message');
        ...
        it('should handle errors gracefully', async() => {
            vi.mocked(updateGuildData).mockImplementation(() => {
