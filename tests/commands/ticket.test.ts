...
import type { MockClient, MockCommandInteraction, MockGuild, MockTextChannel } from '../mocks/discord.js';
...
describe('Ticket Command', () => {
    ...
    let createdChannel: MockTextChannel;
    ...
    describe('execute - Success Cases', () => {
        it('should create ticket channel', async() => {
            await ticketCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction);
        ...
        it('should increment ticket number', async() => {
            vi.mocked(getGuildData).mockResolvedValue({ totalTickets: 5 });
            ...
            const result = callArgs[2]({ totalTickets: 5 }) as unknown as { totalTickets: number };
        ...
        it('should save ticket to open tickets', async() => {
            ...
            const callArgs = vi.mocked(updateGuildData).mock.calls[0]!;
            const result = callArgs[2]({ openTickets: [] }) as unknown as { openTickets: Array<{ userId?: string; reason?: string }> };
        ...
        it('should use category if configured', async() => {
            ...
            vi.mocked(getGuildData).mockResolvedValue({ categoryId: 'category-123' });
        ...
        it('should mention support role if configured', async() => {
            vi.mocked(getGuildData).mockResolvedValue({ supportRoleId: '555666777' });
    ...
    describe('execute - Error Cases', () => {
        ...
        it('should handle missing category gracefully', async() => {
            vi.mocked(getGuildData).mockResolvedValue({ categoryId: 'invalid-category' });
