...
describe('Data Deletion Command', () => {
    ...
    describe('execute - Initial Prompt', () => {
        ...
        it('should reply ephemerally with confirmation embed', async() => {
            await dataDeletionCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction);
        ...
        it('should include Accept and Cancel buttons', async() => {
            ...
            const labels = row.components.map((c: { data: { label: string } }) => c.data.label);
    ...
    describe('execute - Accept Flow', () => {
        beforeEach(() => {
            vi.mocked(getAllGuildData).mockResolvedValue([]);
            vi.mocked(getUserData).mockResolvedValue(undefined);
            vi.mocked(setGuildData).mockResolvedValue();
            vi.mocked(setUserData).mockResolvedValue();
        });
    ...
    describe('execute - Timeout', () => {
        it('should handle collector timeout gracefully', async() => {
            ...
            await expect(dataDeletionCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction)).resolves.not.toThrow();
        });
    });
});
...
describe('deleteUserData helper', () => {
    ...
    it('should count and remove XP/levels from guild_user_store', async() => {
        vi.mocked(getAllGuildData).mockImplementation((store) => {
            if (store === 'levels') {return Promise.resolve([{ guildId: 'guild1', data: [] as unknown as Record<string, unknown> }]);}
            return Promise.resolve([]);
        });
        vi.mocked(getUserData).mockImplementation((store, guildId, userId) => {
