...
import type { MockCommandInteraction, MockGuildMember } from '../mocks/discord.js';
...
interface TagStoredData {
    rules?: { content: string; usageCount: number };
    welcome?: { usageCount: number };
    announce?: { embed: unknown };
    clean?: { embed: unknown };
}
...
describe('Tag Command', () => {
    ...
    let existingTags: Record<string, Record<string, unknown>>;

    beforeEach(() => {
        vi.mocked(getGuildData).mockClear();
        vi.mocked(setGuildData).mockClear();
        ...
        vi.mocked(getGuildData).mockResolvedValue(existingTags);
        vi.mocked(setGuildData).mockResolvedValue();
    ...
    describe('create', () => {
        ...
        it('should create a new tag', async() => {
            ...
            await tagCommand.execute(mockInteraction as unknown as ChatInputCommandInteraction);

            expect(setGuildData).toHaveBeenCalled();
            const [store, guildId, rawData] = vi.mocked(setGuildData).mock.calls[0]!;
            const data = rawData as unknown as TagStoredData;
        ...
        it('should require Manage Messages permission', async() => {
            (mockInteraction.member as unknown as MockGuildMember).permissions.has.mockReturnValue(false);
        ...
        it('should store parsed embed data', async() => {
            ...
            const [, , rawData] = vi.mocked(setGuildData).mock.calls[0]!;
            const data = rawData as unknown as TagStoredData;
