...
describe('utility CLI commands', () => {
    ...
    it('tags list returns tag names', async() => {
        vi.mocked(getGuildData).mockResolvedValue({ greeting: { name: 'greeting', content: 'Hello!', createdByTag: 'user' } });
        const tags = utilityCommands.commands.find(c => c.name === 'tags');
        const list = tags!.subcommands!.find(s => s.name === 'list');
        const result = await list!.execute!({ guild: '123' }) as { count: number; tags: string[] };
    ...
    it('tags show returns tag details', async() => {
        ...
        const tags = utilityCommands.commands.find(c => c.name === 'tags');
        const show = tags!.subcommands!.find(s => s.name === 'show');
        const result = await show!.execute!({ guild: '123', name: 'greeting' }) as { name: string; content: string };
    ...
    it('tags show returns not found for missing tag', async() => {
        ...
        const tags = utilityCommands.commands.find(c => c.name === 'tags');
        const show = tags!.subcommands!.find(s => s.name === 'show');
        const result = await show!.execute!({ guild: '123', name: 'nonexistent' }) as { success: boolean };
    ...
    it('tags create creates a new tag', async() => {
        ...
        const tags = utilityCommands.commands.find(c => c.name === 'tags');
        const create = tags!.subcommands!.find(s => s.name === 'create');
        const result = await create!.execute!({ guild: '123', name: 'newtag', content: 'New content' }) as { success: boolean };
        expect(result.success).toBe(true);
        expect(setGuildData).toHaveBeenCalled();
    });

    it('tags create rejects duplicate', async() => {
        vi.mocked(getGuildData).mockResolvedValue({ newtag: { name: 'newtag' } });
        const tags = utilityCommands.commands.find(c => c.name === 'tags');
        const create = tags!.subcommands!.find(s => s.name === 'create');
        const result = await create!.execute!({ guild: '123', name: 'newtag', content: 'dup' }) as { success: boolean };
        expect(result.success).toBe(false);
    });

    it('tags delete removes a tag', async() => {
        vi.mocked(getGuildData).mockResolvedValue({ oldtag: { name: 'oldtag', content: 'bye' } });
        const tags = utilityCommands.commands.find(c => c.name === 'tags');
        const del = tags!.subcommands!.find(s => s.name === 'delete');
        const result = await del!.execute!({ guild: '123', name: 'oldtag' }) as { success: boolean };
        expect(result.success).toBe(true);
        expect(setGuildData).toHaveBeenCalled();
    });
    ...
    it('tags delete returns not found for missing tag', async() => {
        ...
        const tags = utilityCommands.commands.find(c => c.name === 'tags');
        const del = tags!.subcommands!.find(s => s.name === 'delete');
        const result = await del!.execute!({ guild: '123', name: 'nonexistent' }) as { success: boolean };
        expect(result.success).toBe(false);
    });
