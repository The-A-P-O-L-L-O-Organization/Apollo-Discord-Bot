...
describe('tickets CLI commands', () => {
    ...
    it('list returns open and closed counts', async() => {
        ...
        const list = ticketsCommands.commands!.find(c => c.name === 'list');
        const result = await list!.execute!({ guild: '123' }) as { openCount: number; closedCount: number; open: Array<{ id: string }>; closed: Array<{ ticketNumber: number }> };
    ...
    it('list handles empty data', async() => {
        vi.mocked(getGuildData).mockResolvedValue({});
        const list = ticketsCommands.commands!.find(c => c.name === 'list');
        const result = await list!.execute!({ guild: '123' }) as { openCount: number; closedCount: number };
