...
import type { ModLogOptions } from '../../src/utils/modLog.js';
import {
    createMockUser,
    createMockGuild,
    createMockChannel,
...
describe('ModLog Utility', () => {
    describe('sendModLog', () => {
        ...
        let logChannel: MockTextChannel;
        let targetUser: MockUser;
        let moderatorUser: MockUser;
        ...
        describe('Success Cases', () => {
            ...
            it('should send log embed for mute action with duration', async() => {
                ...
                expect(embed.fields.some((f: { name: string; value: string }) => f.name === 'Duration' && f.value === '1 hour')).toBe(true);
            });
            ...
            it('should include target user information', async() => {
                ...
                const sendCall = logChannel.send.mock.calls[0]![0];
                const embed = sendCall.embeds[0]!;
                const targetField = embed.fields.find((f: { name: string; value: string }) => f.name === 'Target User');
            ...
            it('should include moderator information', async() => {
                ...
                const sendCall = logChannel.send.mock.calls[0]![0];
                const embed = sendCall.embeds[0]!;
                const modField = embed.fields.find((f: { name: string; value: string }) => f.name === 'Moderator');
            ...
            it('should add extra fields when provided', async() => {
                ...
                const sendCall = logChannel.send.mock.calls[0]![0];
                const embed = sendCall.embeds[0]!;
                expect(embed.fields.some((f: { name: string; value: string }) => f.name === 'Messages Deleted')).toBe(true);
                expect(embed.fields.some((f: { name: string; value: string }) => f.name === 'Channel')).toBe(true);
            });
        ...
        describe('Disabled Logging', () => {
            it('should not send log when logging is disabled', async() => {
                ...
                (config.moderation as { logModerationActions: boolean }).logModerationActions = false;
