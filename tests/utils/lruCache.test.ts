...
describe('LRUCache', () => {
    ...
    it('should call onEvict callback', () => {
        ...
        const cache = new LRUCache({
            maxSize: 2,
            onEvict: (key: string, value: number) => evicted.push({ key, value })
        });
...
describe('TwoLevelLRUCache', () => {
    ...
    it('should call onEvict callback', () => {
        const evicted: Array<{ guildId: string; userId: string; value: unknown }> = [];
        const cache = new TwoLevelLRUCache({
            maxGuilds: 1,
            maxUsersPerGuild: 1,
            maxTotalUsers: 1,
            onEvict: (guildId: string, userId: string, value: unknown) => evicted.push({ guildId, userId, value })
        });
