...
describe('XP Utility', () => {
    beforeEach(() => {
        ...
        vi.mocked(setUserData).mockResolvedValue(undefined);
    ...
    describe('awardXp', () => {
        ...
        it('should accumulate XP for an existing user', async() => {
            vi.mocked(getUserData).mockResolvedValue({ xp: 50, level: 0, messages: 3 });
        ...
        it('should level up when XP reaches the next level threshold', async() => {
            vi.mocked(getUserData).mockResolvedValue({ xp: 90, level: 0, messages: 1 });
        ...
        it('should not increment messages when incrementMessages is false', async() => {
            vi.mocked(getUserData).mockResolvedValue({ xp: 10, level: 0, messages: 5 });
    ...
    describe('getLevelsConfig', () => {
        ...
        it('should use guild config overrides', async() => {
            vi.mocked(getGuildData).mockResolvedValue({ enabled: false, cooldown: 5000, minXp: 10, maxXp: 30 });
