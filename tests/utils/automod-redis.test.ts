...
describe('Automod Redis spam tracking', () => {
    ...
    let mockRedis: AutomodRedisMock;
    ...
    beforeEach(async() => {
        ...
        const createPipeline = () => {
            const commands: Array<[string, ...Array<unknown>]> = [];
            return {
                zadd: vi.fn((...args: Array<unknown>) => { commands.push(['zadd', ...args]); return createPipeline(); }),
                zremrangebyscore: vi.fn((...args: Array<unknown>) => { commands.push(['zremrangebyscore', ...args]); return createPipeline(); }),
                zcount: vi.fn((...args: Array<unknown>) => { commands.push(['zcount', ...args]); return createPipeline(); }),
                expire: vi.fn((...args: Array<unknown>) => { commands.push(['expire', ...args]); return createPipeline(); }),
