...
describe('downloadPluginArchive', () => {
    ...
    it('should follow redirects up to the limit', async() => {
        ...
        const responses: Record<string, { status: number; headers: Record<string, string>; body: Buffer | null }> = {
    ...
    it('should abort when too many redirects occur', async() => {
        const loop = (url: string) => ({
            ok: false, status: 302, statusText: 'Found',
            headers: { get: (name: string) => name.toLowerCase() === 'location' ? 'https://loop.example.com/again.zip' : null },
            url,
