...
describe('Global store (getData/setData)', () => {
  ...
  beforeAll(async () => {
    ...
    await db.schema.createTable('guild_store', (t) => {
      (t.text('store') as unknown as { notNull: () => unknown }).notNull();
      (t.text('guild_id') as unknown as { notNull: () => unknown }).notNull();
      (t.text('data') as unknown as { notNull: () => { defaultTo: (v: string) => unknown } }).notNull().defaultTo('{}');
