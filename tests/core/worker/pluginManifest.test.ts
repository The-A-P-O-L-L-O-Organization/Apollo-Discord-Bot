...
import type { ParsePluginManifestOptions } from '../../../src/core/worker/pluginManifest.js';
...
describe('pluginManifest', () => {
    ...
    it('should parse a manifest file', async() => {
        ...
        })) as unknown as ParsePluginManifestOptions['readFile'] });
    ...
    it('should fail when manifest declares no capabilities', async() => {
        await expect(parsePluginManifest({ dir: '/tmp/x', readFile: (async () => JSON.stringify({ id: 'test' })) as unknown as ParsePluginManifestOptions['readFile'] }))
            .rejects.toThrow(/capabilities/i);
    });

    it('should fail when manifest has no id', async() => {
        await expect(parsePluginManifest({ dir: '/tmp/x', readFile: (async () => JSON.stringify({ capabilities: ['api:getOwnConfig'] })) as unknown as ParsePluginManifestOptions['readFile'] }))
